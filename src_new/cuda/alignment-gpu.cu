...
void msa::progressive::gpu::GPU_pointers::allocateMemory(Option *option, int len, int numBlocks) {
    ...
    cudaMalloc((void **)&(this->devicePointers), this->memBlock * sizeof(float*));
    // Allocate memory
    ...
    cudaMalloc((void **)&(this->deviceGapOp), 2 * len * numBlocks * sizeof(float));
    cudaMalloc((void **)&(this->deviceGapEx), 2 * len * numBlocks * sizeof(float));
    cudaMalloc((void **)&(this->deviceLen), 2 * numBlocks * sizeof(int32_t));
    cudaMalloc((void **)&(this->deviceNum), 2 * numBlocks * sizeof(int32_t));
    cudaMalloc((void **)&(this->deviceAln), 2 * len * numBlocks * sizeof(int8_t));
...
void msa::progressive::gpu::GPU_pointers::freeMemory()
{
    // free memory
    cudaSetDevice(this->gpu_index);
    for (int i = 0; i < this->memBlock; ++i) cudaFree(this->deviceFreq[i]);
    cudaFree(this->deviceGapOp);
    cudaFree(this->deviceGapEx);
    cudaFree(this->deviceLen);
    cudaFree(this->deviceNum);
    cudaFree(this->deviceAln);
    cudaFree(this->deviceAlnLen);
    cudaFree(this->deviceSeqInfo);
    cudaFree(this->deviceParam);
    cudaFree(this->devicePointers);
...
void msa::progressive::gpu::GPU_pointers::memcpyHost2Device(Option *option, int len, int alnPairs, int numBlocks)
{
    ...
    cudaMemcpy(this->devicePointers, this->hostPointers,              this->memBlock * sizeof(float*),   cudaMemcpyHostToDevice);
    cudaMemcpy(this->deviceGapOp,    this->hostGapOp,              2 * len * alnPairs * sizeof(float),   cudaMemcpyHostToDevice);
    cudaMemcpy(this->deviceGapEx,    this->hostGapEx,              2 * len * alnPairs * sizeof(float),   cudaMemcpyHostToDevice);
    cudaMemcpy(this->deviceAln,      this->hostAln,                2 * len * alnPairs * sizeof(int8_t),  cudaMemcpyHostToDevice);
...
{
    cudaSetDevice(this->gpu_index);
    cudaMemcpy(this->hostAln,    this->deviceAln,    2 * len * alnPairs * sizeof(int8_t),  cudaMemcpyDeviceToHost);
