...
void msaCpu(Tree* tree, std::vector<std::pair<Node*, Node*>>& nodes, msa::utility* util, msa::option* option, Params& param)
{
    ...
    for (int nIdx = range.begin(); nIdx < range.end(); ++nIdx) {
        ...
        // ======== MAFFT 's approach (find seeds)
        
        if (nIdx == 0) {
        std::cout << nodes[nIdx].first->identifier << ':' << nodes[nIdx].second->identifier << '\n';
        std::vector<double> pRef, pQry, vRef, vQry; 
        std::vector<std::complex<double>> pRef_FFT, pQry_FFT, vRef_FFT, vQry_FFT;
        std::vector<std::complex<double>> p_corr_FFT, v_corr_FFT;
        std::vector<double> p_corr, v_corr;
        size_t fftSize = std::max(freqRef.size(), freqQry.size());
        compute_vec(freqRef, pRef, vRef, fftSize);
        compute_vec(freqQry, pQry, vQry, fftSize);
        r2cFFT(pRef, pRef_FFT);
        r2cFFT(vRef, vRef_FFT);
        r2cFFT(pQry, pQry_FFT);
        r2cFFT(vQry, vQry_FFT);
        conjugate_and_multiply(pRef_FFT, pQry_FFT, p_corr_FFT);
        conjugate_and_multiply(vRef_FFT, vQry_FFT, v_corr_FFT);
        c2rFFT(p_corr_FFT, p_corr);
        c2rFFT(v_corr_FFT, v_corr);
        std::vector<double> corr (p_corr.size(), 0.0);
        for (int i = 0; i < p_corr.size(); ++i) corr[i] = p_corr[i] + v_corr[i];
        std::cout << freqRef.size() << ',' << freqQry.size() << ':' << p_corr.size() << '\n';
        std::vector<int> peaks;
        getPeaks(corr, peaks);
        std::vector<Seed*> seeds, chainedSeeds;
        getSeeds(freqRef, freqQry, peaks, seeds);
        chainSeeds(seeds, chainedSeeds);
        std::vector<Segment*> segments; 
        std::pair<int,int> newLens = std::make_pair(newRef, newQry);
        getAlnSegments(chainedSeeds, segments, newLens);
        std::vector<std::vector<int8_t>> newAlign;
        Talco_xdrop::Params* talco_params = new Talco_xdrop::Params(hostParam, param.matrixSize);
        for (int i = 0; i < segments.size(); ++i) {
            auto ra = realigns[i];
            bool reverse = (i == 0 && segments.size() > 1);
            int16_t errorType = 0;
            std::vector<int8_t> t_aln;
            std::vector<std::vector<float>> t_freqRef ((ra->re - ra->rs + 1), std::vector<float>(profileSize, 0.0));
            std::vector<std::vector<float>> t_freqQry ((ra->qe - ra->qs + 1), std::vector<float>(profileSize, 0.0));
            std::vector<std::vector<float>> t_gapOp (2), t_gapEx (2);    
            if (reverse) {
                for (int s = ra->rs; s < ra->re+1; s++) for (int t = 0; t < profileSize; ++t) t_freqRef[s - ra->rs][t] = freqRef[s][t];
                for (int s = ra->qs; s < ra->qe+1; s++) for (int t = 0; t < profileSize; ++t) t_freqQry[s - ra->qs][t] = freqQry[s][t];     
            }
            for (int r = ra->rs; r < ra->re+1; ++r) {
                t_gapOp[0].push_back(gapOp[0][r]);
                t_gapEx[0].push_back(gapEx[0][r]);
            }
            for (int q = ra->qs; q < ra->qe+1; ++q) {
                t_gapOp[1].push_back(gapOp[1][q]);
                t_gapEx[1].push_back(gapEx[1][q]);
            }
            if (t_freqRef.size() == 0) for (int a = 0; a < t_freqQry.size(); ++a) t_aln.push_back(1);
            if (t_freqQry.size() == 0) for (int a = 0; a < t_freqRef.size(); ++a) t_aln.push_back(2);
            while (t_aln.empty()) {
                Talco_xdrop::Align_freq (
                    talco_params,
                    t_freqRef,
            ...
            if (refNum == 1 && qryNum == 1) {
                std::cout << nodes[nIdx].first->identifier << '\t' << nodes[nIdx].second->identifier << '\n';
    
                int rIdx = tree->allNodes[nodes[nIdx].first->identifier]->msaIdx[0];
                int qIdx = tree->allNodes[nodes[nIdx].second->identifier]->msaIdx[0];
                int storage_r = util->seqsStorage[rIdx];
                int storage_q = util->seqsStorage[qIdx];
                for (int r = ra->rs; r <= ra->re; ++r) std::cout << util->alnStorage[storage_r][rIdx][r];
                std::cout << '\n';
                for (int q = ra->qs; q <= ra->qe; ++q) std::cout << util->alnStorage[storage_q][qIdx][q];
                std::cout << '\n';
                std::cout << "Old: \n";
                for (int a = regions[i].first; a < regions[i].second+1; ++a) {
                    std::cout << (aln_old[a] & 0xFFFF);
                }
                std::cout << '\n';
                std::cout << "New: \t" << t_freqRef.size() << '\t' << t_freqQry.size() << '\n';
                for (auto a: t_aln) {
                    std::cout << (a & 0xFFFF);
                }
                std::cout << '\n';
        ...
        if (!aln_old.empty()) {
            ...
            if (util->nowProcess > 2) {
                // std::cout << nodes[nIdx].first->identifier << '\t' << nodes[nIdx].second->identifier << '\n';
                int m_size = (option->type == 'n') ? 6 : 22;
                std::vector<std::vector<float>> mergeFreq(aln_old.size(), std::vector<float>(m_size,0.0));
                int rIdx = 0, qIdx = 0;
                for (int j = 0; j < aln_old.size(); ++j) {
                    if (aln_old[j] == 0) {
                        for (int k = 0; k < m_size; ++k) mergeFreq[j][k] = (freqRef[rIdx][k] / refNum * refWeight) + (freqQry[qIdx][k] / qryNum * qryWeight);
                        ++rIdx; ++qIdx;
                    }
                    else if (aln_old[j] == 1) {
                        for (int k = 0; k < m_size-1; ++k) mergeFreq[j][k] = (freqQry[qIdx][k] / qryNum * qryWeight);
                        mergeFreq[j][m_size-1] = (freqQry[qIdx][m_size-1] / qryNum * qryWeight) + 1.0 * refWeight;
                        ++qIdx;
                    }
                    else if (aln_old[j] == 2) {
                        for (int k = 0; k < m_size-1; ++k) mergeFreq[j][k] = (freqRef[rIdx][k] / refNum * refWeight);
                        mergeFreq[j][m_size-1] = (freqRef[rIdx][m_size-1] / refNum * refWeight) + 1.0 * qryWeight;
                        ++rIdx;
                    }
                }
                std::vector<bool> conserved (aln_old.size(), false);
                // for (auto a: aln_old) {
                //     std::cout << (a & 0xFFFF);
                // }
                // std::cout << '\n';
                conservedColumns(mergeFreq, conserved, (refWeight+qryWeight));
                std::vector<reAlign*> realigns;
                std::vector<std::pair<int,int>> regions;
                getReAlignRegion(aln_old, conserved, realigns, regions);
                if (!realigns.empty()) {
                    std::vector<std::vector<int8_t>> newAlign;
                    Talco_xdrop::Params* talco_params = new Talco_xdrop::Params(hostParam, param.matrixSize);
                    talco_params->gapBoundary = 0;
                    for (int i = 0; i < realigns.size(); ++i) {
                        auto ra = realigns[i];
                        int16_t errorType = 0;
                        std::vector<int8_t> t_aln;
                        std::vector<std::vector<float>> t_freqRef ((ra->re - ra->rs + 1), std::vector<float>(profileSize, 0.0));
                    ...
                    delete talco_params;
                    std::vector<int8_t> aln_temp;
                    int regionIdx = 0;
                    for (int i = 0; i < aln_old.size(); ++i) {
                        if (regionIdx == regions.size()) {
                            for (i = i; i < aln_old.size(); ++i) aln_temp.push_back(aln_old[i]);
                            break;
                        }
                        if (i < regions[regionIdx].first || i > regions[regionIdx].second) aln_temp.push_back(aln_old[i]);
                        else if (i == regions[regionIdx].second) {
                            for (auto a: newAlign[regionIdx]) aln_temp.push_back(a);
                            regionIdx++;
                        }
                    }
                    // std::cout << "=================== ORIGIN =============\n";
                    ...
                    aln_temp.clear();
                    for (auto ra: realigns) delete ra;
                }
            }
            */
            // =============
