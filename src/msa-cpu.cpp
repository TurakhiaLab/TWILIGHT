...
void calculateProfileFreq(float* profile, Tree* tree, std::pair<Node*, Node*>& nodes, msa::utility* util, char type, int32_t profileLen, int32_t profileSize, std::pair<int, int> startPos) {
    ...
    if (util->nowProcess < 2) {
        float refWeight = 0.0, qryWeight = 0.0;
        int32_t numThreshold = PROFILE_LEN_TH;
    ...
    if (option->psgop) {
        for (int s = 0; s < seqLen; ++s) {
            if (s < refLen) {
                ...
                if (gapRatio > 0) {
                    hostGapOp[offsetg+s] = param.gapOpen * 0.3 * ((refNum-gapRatio)*1.0 / refNum);
        ...
        for (int s = 0; s < seqLen; ++s) {
            if (s < qryLen) {
                ...
                if (gapRatio > 0) {
                    hostGapOp[offsetg+seqLen+s] = param.gapOpen * 0.3 * ((qryNum-gapRatio) * 1.0 / qryNum);
