...
void transitivityMerge_cpu_mod(Tree* tree, Tree* newtree, std::vector<std::pair<Node*, Node*>> nodes, msa::utility* util)
{
    ...
    for (auto n: nodes) {
        // std::cout << tree->allNodes[n.first->identifier]->identifier << ',' << tree->allNodes[n.second->identifier]->identifier << '\n';
        preOrderList.push_back(newtree->allNodes[n.second->identifier]);
        // std::cout << "Level:" << n.first->level << ',' << n.second->level << '\n';
        if (n.first->parent == nullptr) {
            // tree->allNodes[n.first->identifier]->msa.push_back(n.first->identifier);
            for (auto id: tree->allNodes[n.second->identifier]->msa) tree->allNodes[n.first->identifier]->msa.push_back(id);
    ...
    // preOrderList.push_back(newtree->root);
    // for (auto idd: preOrderList) std::cout << idd->identifier << '(' << idd->level << ')' << ',';
    // std::cout << '\n';
    for (int i = nodes.size()-1; i >= 0; --i) { 
        auto n = nodes[i];
        // std::cout << tree->allNodes[n.first->identifier] << '\n';
        std::vector<int8_t> newAln, oldAln = tree->allNodes[n.second->identifier]->msaAln;
        int oldIdx = 0;
        // std::cout << "Aln Len: " << oldAln.size() << ',';
        for (int j = 0; j < tree->allNodes[n.first->identifier]->msaAln.size(); ++j) {
            // if ((tree->allNodes[preOrderList[i]->identifier]->msaAln[j] & 0XFFFF) == 3 && ((oldAln[oldIdx] & 0XFFF) != 3 && (oldAln[oldIdx] & 0XFFF) != 1)) {
            // if (tree->allNodes[n.second->identifier]->identifier == "node_25" || tree->allNodes[n.second->identifier]->identifier == "node_55") {
            //     if ((tree->allNodes[n.first->identifier]->msaAln[j] & 0XFFFF) == 3) {
            //         std::cout << "j: " << j << " oldIdx: " << oldIdx << '(' << (oldAln[oldIdx] & 0xFFFF) << ')' << '\n';
            //     }
            //     if ((oldAln[oldIdx] & 0XFFFF) == 3) {
            //         std::cout << "j: " << j << " OldIdx: " << oldIdx << '(' << (oldAln[oldIdx] & 0xFFFF) << ')' << '\n';
            //     }
            // }
            if ((tree->allNodes[n.first->identifier]->msaAln[j] & 0XFFFF) == 3 && ((oldAln[oldIdx] & 0XFFF) == 0)) {
                newAln.push_back(3);
            }
            else if ((tree->allNodes[n.first->identifier]->msaAln[j] & 0XFFFF) == 3 && ((oldAln[oldIdx] & 0XFFF) == 2)) {
                
                int consecGap = 0;
                while (((tree->allNodes[n.first->identifier]->msaAln[j] & 0xFFFF) == 3 && j < tree->allNodes[n.first->identifier]->msaAln.size())) {
                    ++consecGap;
                    ++j;
                }
                --j;
                int consecIns = 0;
                while (((oldAln[oldIdx] & 0xFFFF) == 2 || (oldAln[oldIdx] & 0xFFFF) == 3) && oldIdx < oldAln.size()) {
                    ++consecIns;
                    ++oldIdx;
                }
                if (consecGap > consecIns) {
                    for (size_t g = 0; g < consecGap-consecIns; ++g) {
                        newAln.push_back(3);
                    }
                }
                for (size_t g = 0; g < consecIns; ++g) {
                    newAln.push_back(oldAln[oldIdx-consecIns+g]);
                }
                // std::cout << tree->allNodes[n.second->identifier]->identifier << ':' << j-consecGap+1 << ' ' << consecGap << ',' << consecIns << '\n';
                // newAln.push_back(3);
        ...
        tree->allNodes[tree->allNodes[n.second->identifier]->identifier]->msaAln = newAln;
